...
// Unlike the "WT" family apps (Pickleball/Coffee/Word Traps), this repo's
// scripts (storage.js, config.js) are plain top-level scripts, not IIFEs
// closed over `window` — they reference `localStorage`, `navigator`, etc.
// as bare globals. So this loader exposes window's storage/nav/location
// directly on the vm context too, not just via `context.window.*`.
function loadBrowserScript(scriptRelativePath, options) {
  ...
  const context = {
    ...
    localStorage: windowLike.localStorage,
